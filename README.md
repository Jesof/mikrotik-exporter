...
### Правила фаервола (Labels: router, chain, action, ip_version, section)

| Метрика                         | Тип     | Описание                          |
| ------------------------------- | ------- | --------------------------------- |
| `mikrotik_firewall_rule_bytes`   | counter | Байты, соответствующие правилам фаервола |
| `mikrotik_firewall_rule_packets` | counter | Пакеты, соответствующие правилам фаервола |

Метрики фаервола собираются для всех активных правил в разделах filter, nat, mangle и raw.
Метка `section` указывает на раздел фаервола:
